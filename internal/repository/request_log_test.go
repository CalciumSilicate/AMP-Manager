...
func TestGetByIDWithJoinsNormalizesHistoricalUnderReservedStatus(t *testing.T) {
	setupRequestLogTestDB(t)
	...
	_, err := db.Exec(`
		INSERT INTO request_logs (
			...
			is_streaming, input_tokens, output_tokens, cost_micros, cost_usd,
			charged_subscription_micros, charged_balance_micros, billing_status
		) VALUES (?, ?, ?, ?, ?, 'POST', '/v1/responses', 200, 123, 0, 100, 20, 140, '0.000140', 100, 10, 'under_reserved')
	`, "log-billing-legacy", now, "user-1", "key-user-1", "gpt-5.4")
	if err != nil {
		t.Fatalf("insert request log failed: %v", err)
	}

	repo := NewRequestLogRepository()
	logEntry, err := repo.GetByIDWithJoins("log-billing-legacy")
	if err != nil {
		t.Fatalf("GetByIDWithJoins returned error: %v", err)
	}
	if logEntry == nil {
