...
func (r *RequestLogRepository) List(params ListParams) ([]model.RequestLog, int64, error) {
	...
	for rows.Next() {
		...
		var originalModel, mappedModel, provider, channelID, channelName, endpoint, errorType, requestID, costUsd, pricingModel, thinkingLevel, downstreamTransport, upstreamTransport, transportFallbackReason, outputPreview sql.NullString
...
func (r *RequestLogRepository) GetByID(id string) (*model.RequestLog, error) {
	...
	var status sql.NullString
	var isStreaming int
	var originalModel, mappedModel, provider, channelID, channelName, endpoint, errorType, requestID, costUsd, pricingModel, thinkingLevel, downstreamTransport, upstreamTransport, transportFallbackReason sql.NullString
	var inputTokens, outputTokens, cacheRead, cacheCreation, costMicros, ttfbMs sql.NullInt64
