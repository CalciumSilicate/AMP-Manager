...
func (r *ChannelRepository) Create(channel *model.Channel) error {
	...
	_, err := db.Exec(
		`INSERT INTO channels (id, type, endpoint, name, base_url, api_key, enabled, weight, priority, model_whitelist, simulate_cli, simulate_system_prompt, traditional_chinese, copilot_api, models_json, headers_json, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		channel.ID, channel.Type, channel.Endpoint, channel.Name, channel.BaseURL, channel.APIKey,
		channel.Enabled, channel.Weight, channel.Priority, channel.ModelWhitelist, channel.SimulateCLI, channel.SimulateSystemPrompt, channel.TraditionalChinese, channel.CopilotAPI, channel.ModelsJSON, channel.HeadersJSON,
...
func (r *ChannelRepository) GetByID(id string) (*model.Channel, error) {
	...
	err := db.QueryRow(
		`SELECT id, type, endpoint, name, base_url, api_key, enabled, weight, priority, model_whitelist, simulate_cli, simulate_system_prompt, traditional_chinese, copilot_api, models_json, headers_json, created_at, updated_at
...
func (r *ChannelRepository) Update(channel *model.Channel) error {
	...
	_, err := db.Exec(
		`UPDATE channels SET type = ?, endpoint = ?, name = ?, base_url = ?, api_key = ?, enabled = ?, weight = ?, priority = ?, model_whitelist = ?, simulate_cli = ?, simulate_system_prompt = ?, traditional_chinese = ?, copilot_api = ?, models_json = ?, headers_json = ?, updated_at = ?
		 WHERE id = ?`,
		channel.Type, channel.Endpoint, channel.Name, channel.BaseURL, channel.APIKey, channel.Enabled, channel.Weight, channel.Priority, channel.ModelWhitelist, channel.SimulateCLI, channel.SimulateSystemPrompt, channel.TraditionalChinese, channel.CopilotAPI, channel.ModelsJSON, channel.HeadersJSON, channel.UpdatedAt,
		channel.ID,
	)
	return err
}
