const NOTO_SANS_SC_FULL_READY_KEY = 'ampmanager:font:noto-sc-full-ready'
...
function markNotoSansScFullFontReady() {
  try {
    ...
    // Storage failures should not block the font upgrade path.
  }
}
...
async function waitForStylesheetLoad(link: HTMLLinkElement) {
  ...
  if (link.dataset.error === '1') {
    throw new Error('Noto Sans SC stylesheet failed to load.')
  }
...
export async function loadNotoSansScFullFont() {
  ...
  if (isNotoSansScFullFontReady()) {
    return true
  }
  if (fullFontLoadPromise) {
